...
constexpr int imu_packet_size_ = 48;
constexpr int64_t encoder_ticks_per_rev_ = 90112;
...
static const ProfileEntry& lookup_profile_entry(UDPProfileLidar profile) {
    ...
    auto it =
        std::find_if(profiles.begin(), end,
...
struct packet_format::Impl {
    ...
    std::map<ChanField, FieldInfo> fields;
    ...
    Impl(UDPProfileLidar profile, int pixels_per_column,
        ...
        const auto& entry = lookup_profile_entry(profile);
