...
static std::vector<std::pair<ChanField, ChanFieldType>>
lookup_scan_fields(UDPProfileLidar profile)
{
  auto end = default_scan_fields.end();
  auto it = std::find_if(
          default_scan_fields.begin(), end,
...
LidarScan::LidarScan(size_t w, size_t h, LidarScanFieldTypes field_types)
    : timestamp_{Header<uint64_t>::Zero(w)},
      ...
      status_{Header<uint32_t>::Zero(w)}, field_types_{std::move(field_types)},
      w{static_cast<std::ptrdiff_t>(w)}, h{static_cast<std::ptrdiff_t>(h)}
{
...
LidarScan::LidarScan(size_t w, size_t h, OS1::UDPProfileLidar profile)
    : LidarScan{w, h, lookup_scan_fields(profile)}
{}
...
OS1::ShotLimitingStatus LidarScan::shot_limiting() const
{
  return static_cast<OS1::ShotLimitingStatus>(
          (frame_status &
           frame_status_masks::FRAME_STATUS_SHOT_LIMITING_MASK) >>
          frame_status_shifts::FRAME_STATUS_SHOT_LIMITING_SHIFT);
}

OS1::ThermalShutdownStatus LidarScan::thermal_shutdown() const
{
  return static_cast<OS1::ThermalShutdownStatus>(
          (frame_status &
...
Eigen::Ref<const LidarScan::Header<uint16_t>> LidarScan::measurement_id() const
{
  return measurement_id_;
}
...
bool LidarScan::complete(OS1::ColumnWindow window) const
{
...
LidarScanFieldTypes get_field_types(const OS1::sensor_info &info)
{
  ...
  return lookup_scan_fields(info.format.udp_profile_lidar);
}
...
std::string to_string(const LidarScanFieldTypes &field_types)
{
  ...
  for (size_t i = 0; i < field_types.size(); ++i) {
    if (i > 0) ss << ", ";
    ss << OS1::to_string(field_types[i].first) << ":"
       << OS1::to_string(field_types[i].second);
  }
  ss << ")";
  return ss.str();
}
...
std::string to_string(const LidarScan &ls)
{
  ...
  if (!field_types.empty()) {
    ...
    for (const auto &ft: ls) {
      visit_field(ls, ft.first, read_and_cast(), key);
      ss << "    " << to_string(ft.first) << ":" << to_string(ft.second)
         << " = (";
      ss << key.minCoeff() << "; " << key.mean() << "; " << key.maxCoeff();
      ss << ")," << std::endl;
    }
    ss << "  ]," << std::endl;
  }
...
ScanBatcher::ScanBatcher(size_t w, const OS1::packet_format &pf)
    : w(w), h(pf.pixels_per_column), next_valid_m_id(0), next_headers_m_id(0),
      cache(pf.lidar_packet_size), pf(pf)
{}

ScanBatcher::ScanBatcher(const OS1::sensor_info &info)
    : ScanBatcher(info.format.columns_per_frame, OS1::get_format(info))
{}
 ...
 */
void zero_header_cols(const std::shared_ptr<LidarScan> &ls,
...
struct parse_field_col {
  ...
  void operator()(Eigen::Ref<img_t<T>> field, ChanField f, uint16_t m_id,
                  const OS1::packet_format &pf, const uint8_t *col_buf)
  {
 ...
 */
bool raw_headers_enabled(const OS1::packet_format &pf, const LidarScan &ls)
{
  ChanFieldType raw_headers_ft = ls.field_type(ChanField::RAW_HEADERS);
  if (!raw_headers_ft) { return false; }
  // ensure that we can pack headers into the size of a single RAW_HEADERS
  // column
  if (pf.pixels_per_column * OS1::field_type_size(raw_headers_ft) <
      (pf.packet_header_size + pf.col_header_size + pf.col_footer_size +
       pf.packet_footer_size)) {
    std::cerr << "WARNING: Can't fit RAW_HEADERS into a column of "
...
struct pack_raw_headers_col {
  ...
  void operator()(Eigen::Ref<img_t<T>> rh_field, ChanField,
                  const OS1::packet_format &pf, uint16_t col_idx,
...
bool ScanBatcher::operator()(const uint8_t *packet_buf,
                             const std::shared_ptr<LidarScan> &ls)
{
  ...
  if (ls->frame_id == -1) {
    ...
    for (const auto &field_type: *ls) {
      ...
      visit_field(*ls, field_type.first, zero_field_cols(), field_type.first,
                  end_m_id, w);
    }
    ...
    std::cout << "finished cloud from: " << packet_count << " packets" << std::endl;
    packet_count = 0;
    return true;
  }
  ...
  for (int icol = 0; icol < pf.columns_per_packet; icol++) {
    ...
    if (raw_headers) {
      ...
      if (m_id >= next_headers_m_id) {
        visit_field(*ls, ChanField::RAW_HEADERS, zero_field_cols(),
                    ChanField::RAW_HEADERS, next_headers_m_id, m_id);
        next_headers_m_id = m_id + 1;
      }

      visit_field(*ls, ChanField::RAW_HEADERS, pack_raw_headers_col(),
    ...
    foreach_field(*ls, parse_field_col(), m_id, pf, col_buf);
  }
