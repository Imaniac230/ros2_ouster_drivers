/**
 * @file types.cpp
 * @brief Implementation of Ouster client datatypes and constants. 
 */
...
extern const Table<ChanField, const char*, 29> chanfield_strings{{
...
extern const Table<UDPProfileLidar, const char*, 5> udp_profile_lidar_strings{{
...
extern const Table<UDPProfileIMU, const char*, 1> udp_profile_imu_strings{{
        {PROFILE_IMU_LEGACY, "LEGACY"},
}};

extern const Table<ShotLimitingStatus, const char*, 10> shot_limiting_status_strings{{
...
extern const Table<ThermalShutdownStatus, const char*, 2> thermal_shutdown_status_strings{{
...
mat4d default_beam_to_lidar_transform(std::string prod_line) {
  ...
  beam_to_lidar_transform(0, 3) =
          default_lidar_origin_to_beam_origin(std::move(prod_line));
...
template <typename K, typename V, size_t N>
static optional<V> lookup(const Table<K, V, N> table, const K& k) {
  auto end = table.end();
...
static optional<K> rlookup(const Table<K, const char*, N> table,
                           const char* v) {
  auto end = table.end();
...
std::string to_string(ThermalShutdownStatus thermal_shutdown_status) {
  auto res =
          lookup(thermal_shutdown_status_strings, thermal_shutdown_status);
  return res ? res.value() : "UNKNOWN";
}
